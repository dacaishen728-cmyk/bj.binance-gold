...
// สุ่มยอดกำไร/ขาดทุน "ครั้งเดียวตอน settle" แล้วบันทึกลง record (field profit)
// กฎเดียวกันทุกแพ็กเกจ: สุ่มในช่วง base..base*1.8 (ปัด 2 ตำแหน่ง) ไม่ต่ำกว่า base เด็ดขาด
function randomizeProfit(base){base=Number(base)||0;if(base<=0)return 0;return Math.round(base*(1+Math.random()*0.8)*100)/100}
...
export default {async fetch(req,env,ctx){
  ...
  if(path.startsWith('/api/')){
    ...
    if(!rateLimit(req,isAuth?'auth':'api',isAuth?300:1500,15*60*1000)) return json({ok:false,error:'ระบบมีการใช้งานหนาแน่นเกินไป กรุณารอสักครู่แล้วลองใหม่อีกครั้ง (too many requests)'},429,{'Retry-After':'900'});

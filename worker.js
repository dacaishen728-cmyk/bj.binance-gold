...
async function autoSettle(env){
  ...
  try{
    ...
    for(const row of funds.results){
      ...
      const profit=result==='win'?exactFundProfit(amount,pct):-randomizeLoss(amount);
...
async function api(req,env,u){const p=u.pathname.replace(/^\/api\/?/,'').split('/').filter(Boolean);const method=req.method;
  ...
  if(p[0]==='admin'&&p[1]==='funds'&&p[3]==='settle'&&method==='POST'){if(!admin)return json({ok:false,error:'forbidden'},403);const f=await getRecord(env,'funds',p[2]);if(!f)return json({ok:false,error:'not found'},404);if(f.status==='completed')return json({ok:true,fund:f});if(!f.result||!f.endTime||new Date(f.endTime)>new Date())return json({ok:false,error:'fund not due'},409);const amount=Number(f.amount)||0,profit=exactFundProfit(amount,f.percent),x={...f,status:'completed',profit:f.result==='win'?profit:-randomizeLoss(amount),settledAt:now()};const fr=await env.DB.prepare("UPDATE funds SET data=? WHERE id=? AND json_extract(data,'$.status')='active'").bind(JSON.stringify(x),f.id).run();if(!fr.meta||!fr.meta.changes){const cur=await getRecord(env,'funds',p[2]);return json({ok:true,fund:cur||f,user:await getUser(env,f.user_id)});}const usr=f.result==='win'?(await balance(env,f.user_id,amount+profit,'fund_settlement',f.id,{result:f.result})).user:await getUser(env,f.user_id);return json({ok:true,fund:x,user:safeUser(usr)})}

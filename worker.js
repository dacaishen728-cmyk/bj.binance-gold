...
const DEFAULT_SETTINGS = {
  siteName:'Binance Gold', site_name:'Binance Gold', welcomeText:'เริ่มปั่นพอร์ตกันเลย', welcome_text:'เริ่มปั่นพอร์ตกันเลย',
...
async function requireSuperAdmin(req,env){const s=await requireSession(req,env,'admin');if(!s)return null;const r=await env.DB.prepare('SELECT id,username,data FROM admins WHERE id=?').bind(s.actor_id).first();if(!r)return null;const d=parse(r.data,{});return d.role==='superadmin'?s:null}
...
async function login(env,type,identifier,password){
  ...
  await env.DB.prepare('INSERT INTO sessions(id_hash,actor_type,actor_id,expires_at,created_at) VALUES(?,?,?,?,?)').bind(h,type,sid,new Date(Date.now()+7*86400000).toISOString(),now()).run();
...
async function autoSettle(env){
  ...
  try{
    ...
    for(const row of trades.results){
      ...
      const profit=result==='win'?amount*pct/100:-amount;
  ...
  try{
    ...
    for(const row of funds.results){
      ...
      const hadResult=(f.result==='win'||f.result==='lose');
      const result=hadResult?f.result:roll();
      const profit=result==='win'?amount*pct/100:-amount;
      const x={...f,id:row.id,user_id:row.user_id,userId:row.user_id,result,resultSetAt:f.resultSetAt||iso,status:'completed',profit,settledAt:iso,autoSettled:!hadResult};
      ...
      if(!r.meta||!r.meta.changes)continue;
      if(result==='win')await balance(env,row.user_id,amount+profit,'fund_settlement',row.id,{result,auto:true});
    }
...
export default {async fetch(req,env,ctx){
  ...
  if(path.startsWith('/api/')){
    const isAuth=/^\/api\/(auth|admin\/login)/.test(path);
    ...
    if(!rateLimit(req,isAuth?'auth':'api',isAuth?40:1500,15*60*1000)) return json({ok:false,error:'too many requests, please try again later'},429,{'Retry-After':'900'});
...
async function api(req,env,u){const p=u.pathname.replace(/^\/api\/?/,'').split('/').filter(Boolean);const method=req.method;
  ...
  if(p[0]==='auth'&&p[1]==='register'&&method==='POST'){const b=await body(req);if(badKeys(b)||!b.name||!b.phone||!b.password||String(b.password).length<4)return json({ok:false,error:'validation failed'},400);const phone=String(b.phone).trim();const ex=await env.DB.prepare('SELECT id FROM users WHERE phone=?').bind(phone).first();if(ex)return json({ok:false,error:'phone already registered'},409);const h=await bcrypt.hash(String(b.password),10),uid=phone,t=now(),data={id:uid,name:String(b.name).trim(),phone,balance:0,vip:1,kycStatus:'none',avatar:null,createdAt:t};await env.DB.prepare('INSERT INTO users(id,phone,password_hash,data,created_at) VALUES(?,?,?,?,?)').bind(uid,phone,h,JSON.stringify(data),t).run();const l=await login(env,'user',phone,String(b.password));await notify(env,'register',uid,'สมาชิกใหม่',`${data.name} สมัครสมาชิก`);return json({ok:true,user:data},201,{'set-cookie':cookie(sessionCookieName('user'),l.token,604800,cookieSecure(req))})}
  if(p[0]==='auth'&&p[1]==='login'&&method==='POST'){const b=await body(req);const l=await login(env,'user',String(b.phone||'').trim(),String(b.password||''));if(!l)return json({ok:false,error:'invalid credentials'},401);return json({ok:true,user:l.user},200,{'set-cookie':cookie(sessionCookieName('user'),l.token,604800,cookieSecure(req))})}
  if(p[0]==='auth'&&p[1]==='me'&&method==='GET'){const s=await requireSession(req,env,'user');if(!s)return json({ok:false,error:'unauthorized'},401);const user=await getUser(env,s.actor_id);return user?json({ok:true,user}):json({ok:false,error:'unauthorized'},401)}
  ...
  if(p[0]==='admin'&&p[1]==='login'&&method==='POST'){const b=await body(req);const l=await login(env,'admin',String(b.username||'').trim(),String(b.password||''));if(!l)return json({ok:false,error:'invalid credentials'},401);await audit(env,'admin:'+l.user.id,'admin_login',l.user.username,{},req.headers.get('CF-Connecting-IP'));return json({ok:true,admin:l.user},200,{'set-cookie':cookie(sessionCookieName('admin'),l.token,604800,cookieSecure(req))})}
  ...
  if(p[0]==='wallet'&&p[1]==='trades'&&p[3]==='settle'&&method==='POST'){if(!user)return json({ok:false,error:'unauthorized'},401);const t=await getRecord(env,'trades',p[2]);if(!t)return json({ok:false,error:'not found'},404);if(t.user_id!==user.actor_id)return json({ok:false,error:'forbidden'},403);if(t.status==='closed')return json({ok:true,trade:t,user:await getUser(env,user.actor_id)});if(!t.endTime||new Date(t.endTime)>new Date())return json({ok:false,error:'trade not due'},409);let result=t.result;let autoSettled=false;if(result!=='win'&&result!=='lose'){let winRate=50;try{const wr=await env.DB.prepare("SELECT value FROM settings WHERE key='autoWinRate'").first();const wv=Number(parse(wr?.value,wr?.value));if(Number.isFinite(wv)&&wv>=0&&wv<=100)winRate=wv}catch(e){}result=Math.random()*100<winRate?'win':'lose';autoSettled=true}const amount=Number(t.amount)||0,pct=Number(t.percent||(t.package&&t.package.percent)||0),profit=result==='win'?amount*pct/100:-amount;await env.DB.prepare("UPDATE trades SET data=? WHERE id=? AND json_extract(data,'$.status')='pending'").bind(JSON.stringify({...t,result,resultSetAt:t.resultSetAt||now(),status:'closed',profit,closed_at:now(),autoSettled}),t.id).run();const u=result==='win'?(await balance(env,user.actor_id,amount+profit,'trade_settlement',t.id,{result,auto:autoSettled})).user:await getUser(env,user.actor_id);return json({ok:true,trade:{...t,result,status:'closed',profit},user:u})}
  if(p[0]==='wallet'&&p[1]==='funds'&&p[3]==='settle'&&method==='POST'){if(!user)return json({ok:false,error:'unauthorized'},401);const f=await getRecord(env,'funds',p[2]);if(!f)return json({ok:false,error:'not found'},404);if(f.user_id!==user.actor_id)return json({ok:false,error:'forbidden'},403);if(f.status==='completed')return json({ok:true,fund:f,user:await getUser(env,user.actor_id)});if(!f.endTime||new Date(f.endTime)>new Date())return json({ok:false,error:'fund not due'},409);let fresult=f.result,autoF=false;if(fresult!=='win'&&fresult!=='lose'){let winRate=50;try{const wr=await env.DB.prepare("SELECT value FROM settings WHERE key='autoWinRate'").first();const wv=Number(parse(wr?.value,wr?.value));if(Number.isFinite(wv)&&wv>=0&&wv<=100)winRate=wv}catch(e){}fresult=Math.random()*100<winRate?'win':'lose';autoF=true}const amount=Number(f.amount)||0,profit=amount*(Number(f.percent)||0)/100,x={...f,result:fresult,resultSetAt:f.resultSetAt||now(),status:'completed',profit:fresult==='win'?profit:-amount,settledAt:now(),autoSettled:autoF};await env.DB.prepare("UPDATE funds SET data=? WHERE id=? AND json_extract(data,'$.status')='active'").bind(JSON.stringify(x),f.id).run();const usr=fresult==='win'?(await balance(env,f.user_id,amount+profit,'fund_settlement',f.id,{result:fresult,auto:autoF})).user:await getUser(env,f.user_id);return json({ok:true,fund:x,user:usr})}
  ...
  if(p[0]==='admin'&&p[1]==='trades'&&p[3]==='settle'&&method==='POST'){if(!admin)return json({ok:false,error:'forbidden'},403);const t=await getRecord(env,'trades',p[2]);if(!t)return json({ok:false,error:'not found'},404);if(t.status==='closed')return json({ok:true,trade:t});const b=await body(req),result=b.result==='win'?'win':'lose',amount=Number(t.amount)||0,profit=result==='win'?amount*(Number(t.percent||(t.package&&t.package.percent)||0))/100:-amount;const x={...t,status:'closed',profit,closed_at:now()};await env.DB.prepare('UPDATE trades SET data=? WHERE id=?').bind(JSON.stringify(x),t.id).run();const usr=result==='win'?(await balance(env,t.user_id,amount+profit,'trade_settlement',t.id,{result})).user:await getUser(env,t.user_id);await audit(env,'admin:'+admin.actor_id,'settle_trade',t.id,{result},req.headers.get('CF-Connecting-IP'));return json({ok:true,trade:x,user:usr})}
  if(p[0]==='admin'&&p[1]==='funds'&&p[3]==='set-result'&&method==='POST'){if(!admin)return json({ok:false,error:'forbidden'},403);const f=await getRecord(env,'funds',p[2]);if(!f)return json({ok:false,error:'not found'},404);const b=await body(req),x={...f,result:b.result==='win'?'win':'lose',resultSetAt:now()};await env.DB.prepare('UPDATE funds SET data=? WHERE id=?').bind(JSON.stringify(x),f.id).run();await audit(env,'admin:'+admin.actor_id,'set_fund_result',f.id,{result:x.result},req.headers.get('CF-Connecting-IP'));return json({ok:true,fund:x})}
  if(p[0]==='admin'&&p[1]==='funds'&&p[3]==='settle'&&method==='POST'){if(!admin)return json({ok:false,error:'forbidden'},403);const f=await getRecord(env,'funds',p[2]);if(!f)return json({ok:false,error:'not found'},404);if(f.status==='completed')return json({ok:true,fund:f});if(!f.result||!f.endTime||new Date(f.endTime)>new Date())return json({ok:false,error:'fund not due'},409);const amount=Number(f.amount)||0,profit=amount*(Number(f.percent)||0)/100,x={...f,status:'completed',profit:f.result==='win'?profit:-amount,settledAt:now()};await env.DB.prepare('UPDATE funds SET data=? WHERE id=?').bind(JSON.stringify(x),f.id).run();const usr=f.result==='win'?(await balance(env,f.user_id,amount+profit,'fund_settlement',f.id,{result:f.result})).user:await getUser(env,f.user_id);return json({ok:true,fund:x,user:usr})}
  if(COLLECTIONS.includes(p[0])){if(!user&&!admin)return json({ok:false,error:'unauthorized'},401);const col=p[0];if(method==='GET'){
    ...
    return json(await listRecords(env,col,uid,5000))}if(method==='POST'){const b=await body(req);if(badKeys(b))return json({ok:false,error:'invalid field name'},400);
  ...
  if(p[0]==='admins'&&method==='GET'){if(!admin)return json({ok:false,error:'forbidden'},403);const rs=await env.DB.prepare('SELECT * FROM admins ORDER BY created_at ASC').all();return json(rs.results.map(r=>strip({...parse(r.data,{}),id:r.id,username:r.username})))}
  if(p[0]==='admins'&&method==='POST'){if(!await requireSuperAdmin(req,env))return json({ok:false,error:'superadmin required'},403);const b=await body(req);if(!b.username||!b.password||String(b.password).length<4)return json({ok:false,error:'validation failed'},400);const h=await bcrypt.hash(String(b.password),10),rid=id('A'),t=now(),data={id:rid,username:String(b.username).trim(),name:b.name||'',role:b.role||'admin',createdAt:t};try{await env.DB.prepare('INSERT INTO admins(id,username,password_hash,data,created_at) VALUES(?,?,?,?,?)').bind(rid,data.username,h,JSON.stringify(data),t).run();await audit(env,'admin:'+admin.actor_id,'create_admin',rid,{username:data.username},req.headers.get('CF-Connecting-IP'));return json(strip(data),201)}catch(e){return json({ok:false,error:'username already exists'},409)}}
  if(p[0]==='admins'&&p.length===2&&method==='DELETE'){if(!await requireSuperAdmin(req,env))return json({ok:false,error:'superadmin required'},403);if(p[1]===admin.actor_id)return json({ok:false,error:'cannot delete yourself'},400);await env.DB.prepare('DELETE FROM admins WHERE id=?').bind(p[1]).run();await audit(env,'admin:'+admin.actor_id,'delete_admin',p[1],{},req.headers.get('CF-Connecting-IP'));return json({ok:true})}
